...
// Controlla se è necessario resettare le prenotazioni
...
function resetAllReservations() {
  ...
  db.collection("reservations")
    ...
    .then(snapshot => {
      ...
      // Primo passaggio: recupera tutti gli utenti con ruolo admin
      return db.collection("users").where("role", "==", "admin").get().then(adminSnapshot => {
        adminSnapshot.forEach(doc => {
          userIdsToExclude.push(doc.id);
        });

        // Secondo passaggio: elimina le prenotazioni non amministrative
